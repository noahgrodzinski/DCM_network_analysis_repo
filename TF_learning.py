...
def create_dataframes():
  ...
  sns.pairplot(train_dataset, y_vars=["h_index"], x_vars=[
  ...
  return train_labels, test_labels, train_dataset, test_dataset, normed_train_data, normed_test_data


# Create and train the network
def train_new_model(train_labels, train_dataset, normed_train_data, EPOCHS=10000, model_save_path:str='code/saved_model/', ):
  def build_model():
    ...
    SGDoptimizer = keras.optimizers.SGD(learning_rate=0.00001, momentum=0.99) # This worked really well with patience=200 and (1024,128,16,16,1) mae~0.7
...
def make_network_statistics_and_graphs(test_labels, normed_test_data, saved_model_path : str = 'code/saved_model/'):
  """"
  ...
  with open('code/readouts&stats/network_accuracy.txt', 'w') as printout:
      stat_sig = (
        ...
        str(coef)+
        ", corresponding to a probability of "+str(p)+"\n")
      accur = (
...
if __name__=="__main__":
  train_labels, test_labels, train_dataset, test_dataset, normed_train_data, normed_test_data = create_dataframes()
  ...
  if model_path:
    ...
    if model_path:
      ...
      make_network_statistics_and_graphs(test_labels, normed_test_data) # Use last model instead